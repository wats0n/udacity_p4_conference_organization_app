...
from models import WishListReport
...
SESSION_GET_BYTYPE = endpoints.ResourceContainer(
    session_type=messages.StringField(1),
    websafeConferenceKey=messages.StringField(2),
)

SESSION_GET_BYSPEAKER = endpoints.ResourceContainer(
...
# - - - Session - - - - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        if(user_id != conf.organizerUserId):
            raise endpoints.BadRequestException("Not Organizer to create Session")
        ...
        taskqueue.add(params={'user_name': data['speaker'],
    ...
    @endpoints.method(SESSION_GET_BYTYPE, SessionForms,
            path='sessionbytype/{websafeConferenceKey}',
            http_method='GET', name='getConferenceSessionsByType')
    def getConferenceSessionsByType(self, request):
        """Return requested session (by websafeConferenceKey with type)."""
        conf = ndb.Key(urlsafe=request.websafeConferenceKey).get()
        if not conf:
            raise endpoints.NotFoundException(
                'No conference found with key: %s' % request.websafeConferenceKey)
        sessions = Session.query(ancestor=conf.key)
        sessions = sessions.filter(Session.session_type == request.session_type)
        return SessionForms(items=[self._copySessionToForm(s)\
         for s in sessions])

    @endpoints.method(SESSION_GET_BYSPEAKER, SessionForms,
            path='sessionbyspeaker',
            http_method='GET', name='getSessionsBySpeake')
    def getSessionsBySpeake(self, request):
...
# - - - WishList - - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(WISHLIST_POST_REQUEST, WishListForm, path='add_wishlist',\
            http_method='POST', name='addSessionToWishList')
    def addSessioinToWishList(self, request):
        """Create new wishlist."""
    ...
    @endpoints.method(WISHLIST_POST_REQUEST, WishListReport,
            path='del_wishlist',
            http_method='GET', name='deleteSessionsInWishList')
...
# - - - Fetured Speaker - - - - - - - - - - - - - - - - - - - -
    ...
    def _setFeaturedSpeaker(request):
        memcache.set("FEATURED_SPEAKER", request.get('user_name'))
        memcache.set("FEATURED_TOPIC", request.get('session_name'))

    @endpoints.method(message_types.VoidMessage, FeaturedSpeaker,
            path='get_featuredspeaker',
            http_method='GET', name='getFeaturedSpeaker')
    def getFeaturedSpeaker(self, request):

        wfs = FeaturedSpeaker()
        
        setattr(wfs, 'user_name', memcache.get("FEATURED_SPEAKER"))
        setattr(wfs, 'session_name', memcache.get("FEATURED_TOPIC"))
            
        wfs.check_initialized()
