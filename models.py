...
class Session(ndb.Model):
    ...
    conf_key        = ndb.StringProperty()
    sess_key        = ndb.StringProperty()

class SessionForm(messages.Message):
    """SessionForm -- SessionForm outbound form message"""
    name            = messages.StringField(1)
    ...
    conf_key        = messages.StringField(8)
    sess_key        = messages.StringField(9)
...
class WishListForm(messages.Message):
    ...
    session_key = messages.StringField(2)

class WishListForms(messages.Message):
    """ConferenceForms -- multiple Conference outbound form message"""
    items = messages.MessageField(WishListForm, 1, repeated=True)

class WishListReport(messages.Message):
    """SessionForm -- SessionForm outbound form message"""
    rpt = messages.StringField(1)
